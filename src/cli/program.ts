// ---------------------------------------------------------------------------
// llm-clash CLI program definition.
//
...
//
// The `Command` instance is built by the exported `createProgram()` factory
// (instead of at module scope) so tests can parse argv in-process; the bin
// entry `./run.ts` calls it and hands control to commander.
//
// Option routing: the program uses commander's positional options mode
// (`enablePositionalOptions`) so flags that appear AFTER a subcommand name
// are parsed by that subcommand. Without it, shared flags like `--output`
// or `--quiet` were silently swallowed by the top-level command and never
// reached `run`/`refine`.
// ---------------------------------------------------------------------------
...
/**
 * Build the full commander program (default command + `refine` + `run`).
 *
 * A fresh `Command` instance is returned on every call so tests can invoke
 * `createProgram().parseAsync([...])` repeatedly without shared state.
 */
export function createProgram(): Command {
  // Load `.env` ourselves so we don't take a hard dep on dotenv. Must run
  // BEFORE any action reads env-derived defaults.
  loadDotEnv();

  const program = new Command();

  // Route flags positionally: program flags before a subcommand name, the
  // subcommand's own flags after it. See the module header for why.
  ...
  program
    ...
    .option("--no-save", "Do not save run artifacts")
    .option("--output <dir>", "Output directory (overrides outputDir from the YAML)")
    .option("--openrouter-api-key <key>", "OpenRouter API key for openrouter:* model specs")
    .option("--quiet", "Suppress progress logging")
    .action(
      async (
        ...
        await runAndPrint({
          ...
          // Only override `saveArtifacts` / `outputDir` when the flag was set
          // on the CLI; otherwise let the YAML decide.
 ...
 *   - The file is read SYNCHRONOUSLY so the rest of the program can rely on
 *     env vars being populated before any adapter is constructed.
 */
